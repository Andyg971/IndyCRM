...
func testResendEmailService(to recipient: String, completion: @escaping (Result<String, Error>) -> Void) {
    let fromEmail = "ton-email@domaine.com" // Remplace par ton email
    let fromName = "Ton Entreprise"
...
// TEST : Exécution du test uniquement en mode DEBUG
#if DEBUG
testResendEmailService(to: "test@email.com") { result in
    switch result {
    case .success(let id):
        print("✅ Email envoyé avec succès ! ID : \(id)")
    case .failure(let error):
        print("❌ Erreur lors de l'envoi de l'email : \(error.localizedDescription)")
    }
}
#endif 